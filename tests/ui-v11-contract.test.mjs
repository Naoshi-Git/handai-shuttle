...
const source = await readFile(new URL("../src/ui-v11.mjs", import.meta.url), "utf8");
const css = await readFile(new URL("../src/ui-v11.css", import.meta.url), "utf8");

await import("../src/ui-v11.mjs");

test("v11 is the final presentation layer", () => {
  assert.match(entry, /import "\.\/ui-v10\.mjs";[\s\S]*import "\.\/ui-v11\.mjs";/);
});
...
test("search timing and condition icons use aligned stroke SVGs", () => {
  assert.match(source, /search-timing-button \.search-summary-icon/);
  assert.match(source, /search-condition-button \.search-summary-icon/);
  assert.match(source, /iconSvg\(name\)/);
  assert.match(source, /clock:/);
  assert.match(source, /sliders:/);
  assert.match(css, /\.search-summary-icon \.v11-icon[\s\S]*19px/);
});

test("icon audit replaces legacy refresh swap and disclosure glyphs", () => {
  assert.match(source, /#refresh-button/);
  assert.match(source, /#swap-button/);
  assert.match(source, /#home-origin-button \.chevron/);
  assert.match(source, /\.tt-compact-chevron/);
