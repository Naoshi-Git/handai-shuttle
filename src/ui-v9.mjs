...
let adRepairQueued = false;

function installStyles() {
  if ($('link[data-ui-v9]')) return;
  const link = document.createElement("link");
  link.rel = "stylesheet";
  link.href = "./ui-v9.css";
  link.dataset.uiV9 = "true";
...
function effectiveSuitaOrigin() {
  ...
  if (
    currentCampus() === "suita" &&
    ["matched", "manual"].includes(button?.dataset.locationState || "") &&
    VALID_SUITA_ORIGINS.has(detected)
  ) {
    return detected;
  }
...
function routeTypeLabel(journey) {
  ...
  if (originCampus === "minoh" || destinationCampus === "minoh") return "直行";
  return "直行";
}
...
function renderPreciseHome() {
  ...
  const suitChip = $('#home-destination-chips [data-home-destination="suita"]');
  if (suitChip) suitChip.textContent = `吹田・${STOPS[preferredSuitaDestination()].shortName}`;
}

function queuePreciseHome(delay = 0) {
...
function syncTimetableDefaults() {
  const originCampus = currentCampus();
  const destinationCampus = cachedDestination(originCampus);
  if (originCampus === "suita") localStorage.setItem(KEYS.currentSuitaStop, effectiveSuitaOrigin());

  const originButton = $(`[data-tt-origin="${originCampus}"]`);
  if (originButton && !originButton.classList.contains("is-active")) originButton.click();

  window.setTimeout(() => {
    const destinationButton = $(`[data-tt-destination="${destinationCampus}"]`);
    if (destinationButton && !destinationButton.classList.contains("is-active")) destinationButton.click();
    if (originCampus === "suita") setSelectValue($("#tt-suita-stop"), effectiveSuitaOrigin());
  }, 30);
}
...
function bindSuitaPreferences() {
  ...
  if (origin && origin.dataset.v9Bound !== "true") {
    origin.dataset.v9Bound = "true";
  ...
  if (arrival && arrival.dataset.v9Bound !== "true") {
    arrival.dataset.v9Bound = "true";
...
function repairOneAd(slot) {
  if (!slot || slot.dataset.v9EdgeFixed === "true") return;
  if (slot.dataset.pngRasterized !== "true") return;
  const image = $(".house-ad-creative img", slot);
  if (!image?.complete || !image.naturalWidth) return;

  const sourceMap = {
    "home-feed": { src: "./assets/ads/house/v2/house-banner-simple-640x89.webp", inset: 3 },
    "timetable-header": { src: "./assets/ads/house/v2/house-banner-simple-640x89.webp", inset: 3 },
    "timetable-inline": { src: "./assets/ads/house/v2/house-banner-simple-640x89.webp", inset: 3 },
    "search-inline": { src: "./assets/ads/house/v2/house-banner-feature-640x213.webp", inset: 5 }
  };
  const spec = sourceMap[slot.dataset.adSlot];
  if (!spec) return;
  ...
  source.onload = () => {
    ...
    const inset = Math.min(spec.inset, Math.floor(Math.min(width, height) / 8));
    ctx.drawImage(source, inset, inset, width - inset * 2, height - inset * 2, 0, 0, canvas.width, canvas.height);
    image.src = canvas.toDataURL("image/png");
    image.dataset.rasterFormat = "png-v9-opaque";
    slot.dataset.v9EdgeFixed = "true";
  };
  source.src = spec.src;
}

function repairAdEdges() {
  $$("[data-ad-slot]").forEach(repairOneAd);
}

function queueAdRepair() {
  if (adRepairQueued) return;
  adRepairQueued = true;
  requestAnimationFrame(() => {
    adRepairQueued = false;
    repairAdEdges();
  });
}

function bindNavigationDefaults() {
  document.addEventListener("click", (event) => {
    const target = event.target.closest?.("button, [data-home-destination], [data-tt-destination]");
    if (!target) return;

    if (target.matches('[data-nav="search"], #search-now-button, #arrival-search-button, #open-search-button')) {
      window.setTimeout(syncSearchDefaults, 50);
    }
    if (target.matches('[data-nav="timetable"]')) {
      window.setTimeout(syncTimetableDefaults, 80);
    }
    if (target.matches("[data-home-destination]")) {
      saveDestination(target.dataset.homeDestination);
      queuePreciseHome(30);
    }
    if (target.matches("[data-tt-destination]")) saveDestination(target.dataset.ttDestination);
    if (target.matches("[data-campus-choice]")) {
      window.setTimeout(() => {
        const origin = currentCampus();
    ...
    if (target.matches("#refresh-button")) queuePreciseHome(40);
  }, true);

  $("#destination-campus")?.addEventListener("change", (event) => {
    saveDestination(event.target.value);
    if (event.target.value === "suita") window.setTimeout(() => setSelectValue($("#destination-stop"), preferredSuitaDestination()), 0);
  });
  $("#swap-button")?.addEventListener("click", () => window.setTimeout(() => saveDestination($("#destination-campus")?.value), 0));
  $("#default-campus")?.addEventListener("change", () => {
    window.setTimeout(() => {
      if ($("#view-search")?.classList.contains("is-active")) syncSearchDefaults();
      if ($("#view-timetable")?.classList.contains("is-active")) syncTimetableDefaults();
      queuePreciseHome();
    }, 40);
  });
}
...
function observeLocationButton() {
  ...
  if (!button) return;
  const observer = new MutationObserver(normalizeLocateButton);
...
function observeDynamicAds() {
  const observer = new MutationObserver(queueAdRepair);
  [$("#view-home"), $("#view-search"), $("#view-timetable")].filter(Boolean).forEach((node) => observer.observe(node, { childList: true, subtree: true }));
  queueAdRepair();
  window.setTimeout(queueAdRepair, 400);
  window.setTimeout(queueAdRepair, 1000);
}
